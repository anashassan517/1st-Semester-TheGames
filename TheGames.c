...
void Level1(char name[50]) {
	int i, line = 35, score = 0,wrong=0,rnd;
	char a[26], ch;
	time_t start1, end;
	FILE *fptr;
	int x = 5;
	...
	gotoxy(70, 20);
	printf("PASS LEVEL 1 TO REACH IN LEVEL 2!");
	sleep(2);
	start1 = time(NULL);
	fptr = fopen("D:\\FAST UNIVERSITY\\SEMESTER 1\\pf project game\\alphabets1.txt", "r");
	if (fptr == NULL) {
		...
		for (i = 0; i < 26; i++) {
			...
			if (y == 33 || wrong>20) {
				...
				printf("\n\n\n\t\t\t\t\t\t\t\t\t\tGAME OVER!");
				printf("\n\t\t\t==========================================================================================================================");
				printf("\n\n\n\t\t\t\t\t\t\t      !!!!!!!!!! YOUR ACCURATE TYPED SCORE : %d !!!!!!!!!!", score);
				printf("\n\n\n\t\t\t\t\t\t\t        !!!!!!!!!! YOUR WRONG TYPED SCORE : %d !!!!!!!!!!", wrong);
				printf("\n\n\n\t\t\t\t\t\t\t\t   TOTAL TIME PLAYED:%.1f SECONDS!", difftime(end, start1));
				if (difftime(end, start1) > 20 || score < 10 || wrong > 5 || i<10) {
					printf("\n\n\n\t\t\t\t\t\t\t\t NEED TO IMPROVE TYPING SPEED!");
				}
				if (difftime(end, start1) <= 20&& score >= 10 && wrong <= 5 && i>=10) {
					printf("\n\n\n\t\t\t\t\t\t\t\t            GOOD TYPING SPEED!");
				}
				if (difftime(end, start1) <= 25 && i == 25) {
		...
		if (ch == a[25]) {
			fclose(fptr);
			Level2(name,score,wrong,start1);
		}
		fclose(fptr);
	}
}
...
void Level2(char name[50],int score,int wrong,time_t start1) {
	system("cls");
	gotoxy(50, 20);
	printf("YOU PASS LEVEL 1 PERFECTLY!!  NOW PASS LEVEL 2 WITH HIGHER SPEED!");
	sleep(3);
	int i, line = 35,rnd;
	char a[60], ch;
	time_t end;
	...
	printf("GAME IS LOADING PLEASE WAIT....");
	sleep(2);
	fptr = fopen("D:\\FAST UNIVERSITY\\SEMESTER 1\\pf project game\\alphabets2.txt", "r");
	if (fptr == NULL) {
		printf("NO FILE FOUND!");
	} else {
		system("cls");
		...
		rnd=(rand() % 50)+65;
			...
			if (y == 33 || i==52|| wrong>35) {
				...
				printf("\n\n\n\t\t\t\t\t\t\t        !!!!!!!!!! YOUR WRONG TYPED SCORE : %d !!!!!!!!!!", wrong);
				printf("\n\n\n\t\t\t\t\t\t\t\t   TOTAL TIME PLAYED:%.1f SECONDS!", difftime(end, start1));
				if (difftime(end, start1) > 170) {
					printf("\n\n\n\t\t\t\t\t\t\t\t NEED TO IMPROVE TYPING SPEED!");
				}
				if (difftime(end, start1) <= 70 && wrong<=15 && i>25) {
					printf("\n\n\n\t\t\t\t\t\t\t\t            GOOD TYPING SPEED!");
				}
				if (difftime(end, start1) <= 50 && i>52&&wrong<=10) {
					printf("\n\n\n\t\t\t\t\t\t\t\tOUTSTANDING TYPING SPEED!");
				}
				printf("\n\n\n\t\t\t\t\t\t\t !!!!!!!!!! '%s' TRY AGAIN NEXT TIME !!!!!!!!!!", strupr(name));
				break;
...
void LuckBase(int *pt, int n, struct member a[], int k) {
	...
	if (cA > cB) {
		...
		printf("                                                            WINNER->->->''%s'' (AWARDED CASH OF RS %d)\n\n", a[0].name, cA);
		printf("                                                            WHILE LOSER ''%s'' (HAVE CASH RS %d)\n\n", a[1].name, cB);
		printf("________________________________________________________________________________________________________________________________________________________________________");
	} else {
		printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                                             ''WINNER OF THE GAME''\n\n\n\n");
		printf("------------------------------------------------------------------------------------------------------------------------------------------------------------------------\n\n");
